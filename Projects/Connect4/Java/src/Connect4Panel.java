 ...
 *
 * You should not modify this class, and your agent should not need to access
 * the methods within it directly.
 */
public class Connect4Panel extends JPanel {

    private Connect4Game myGame;    // the game to display
    private int slotDiameter, slotSpacing; // space between slots
    ...
    public void paint(Graphics g) {
        ...
        g2.setColor(new Color(63,81,181));//BLUE
            ...
            for (int j = 0; j < myGame.getRowCount(); j++) {
                ...
                if (!currentSlot.getIsFilled()) {
                    ...
                    if (currentSlot.getIsRed()) {
                        color = new Color(255, 87, 34);//RED
                    } else {
                        color = new Color(255, 255, 59);//YELLOW
                    }
                }
    ...
    public void drawHighlight(Graphics2D g2, int x, int y) {
        ...
        g2.setColor(new Color(9, 237, 206));
        g2.drawOval(x + 2, y + 2, slotDiameter - 4, slotDiameter - 4);
        g2.setStroke(save);
    }
}
